...
Date: 2025-03-07
Description: Timeline models for experiment management
License: MIT
"""
...
class TimelineManager:
    ...
    def __init__(self, timelines_dir: str = "timelines"):
        ...
        self.timelines: Dict[str, ExperimentTimeline] = {}
    ...
    def _load_timelines(self):
        """Load all timeline files from disk"""
        if not os.path.exists(self.timelines_dir):
            return
            ...
            if filename.endswith('.json'):
                ...
                try:
                    with open(filepath, 'r') as f:
                        timeline = ExperimentTimeline.from_json(f.read())
    ...
    def save_timeline(self, timeline: ExperimentTimeline) -> bool:
        ...
        try:
            self._ensure_timelines_dir()
            filepath = os.path.join(self.timelines_dir, f"{timeline.name}.json")
    ...
    def delete_timeline(self, name: str) -> bool:
        ...
        try:
            if name in self.timelines:
                filepath = os.path.join(self.timelines_dir, f"{name}.json")
...
# Available trial types for the UI
